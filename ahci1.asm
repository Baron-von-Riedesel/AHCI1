...
displaycl proc uses ebx esi edi dwCL:dword
	...
	.while edi < 32
		...
		.if ecx
			invoke printf, CStr("      CL[%u] PRDTL=0x%X flags=0x%X PRDBC=0x%X (PRDTL=length of PRDT)",lf), edi, eax, ecx, [ebx].CHStruct.PRDBC
...
displayport proc uses ebx esi dwPort:dword, dwPortAddr:dword, dwPhysAddr:dword
	...
	invoke printf, CStr("    IS - Interrupt Status=0x%X ([0]=D2H Reg, [1]=PIO Setup, [2]=DMA Setup, ...)",lf), dword ptr [ebx+10h]
	...
	bt esi,24
	setc al
	invoke printf, CStr("    CMD.ATAPI[24]=%u (1=ATAPI device detected)",lf), eax
	...
	invoke printf, CStr("    SACT - SATA Active=0x%X",lf), dword ptr [ebx+34h]
	invoke printf, CStr("    CI - Command Issued=0x%X",lf), dword ptr [ebx+38h]
	pop eax
